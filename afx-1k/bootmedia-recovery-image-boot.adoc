...
summary: Después de instalar el nuevo dispositivo de medio de arranque en su sistema de almacenamiento AFX 1K, puede iniciar el proceso de recuperación automática del medio de arranque para restaurar la configuración desde el nodo asociado. 
---
= Arranca la imagen de recuperación en tu sistema de almacenamiento AFX 1K usando el proceso de arranque de recuperación automático
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Después de instalar el nuevo dispositivo de medio de arranque en su sistema de almacenamiento AFX 1K, puede iniciar el proceso de recuperación automática del medio de arranque para restaurar la configuración desde el nodo asociado.

.Acerca de esta tarea
Durante el proceso de recuperación, el sistema verifica si el cifrado está habilitado e identifica el tipo de cifrado de clave que se está utilizando. Si el cifrado de clave está habilitado, el sistema le guiará por los pasos necesarios para restaurarlo.
...
+
El proceso intenta restaurar los archivos apropiados del nodo asociado y reinicia el nodo. Continúe con el siguiente paso.
...
.El futuro
Una vez que haya restaurado la imagen de ONTAP y el nodo esté activo y proporcionando datos, deberálink:bootmedia-complete-rma.html["Devuelva la pieza fallida a NetApp"] .
