...
summary: Sustituye el controlador de tu sistema FAS2600 retirando el controlador averiado, trasladando los componentes FRU al módulo controlador de recambio, instalándolo en el chasis e iniciando el sistema en modo de mantenimiento. 
---
= Sustituye el controlador en un sistema FAS2600
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Sustituye el controlador de tu sistema FAS2600 retirando el controlador con fallo, moviendo los componentes FRU al módulo de controlador de reemplazo, instalando el módulo de controlador de reemplazo en el chasis y luego arrancando el sistema en modo de mantenimiento.

link:https://youtu.be/2_uWRt_iJqk["Vídeo de sustitución de la controladora FAS2600 AFF"]
...
Para trasladar el módulo de almacenamiento en caché, denominado como la tarjeta M.2 PCIe en la etiqueta del controlador, del módulo controlador antiguo al módulo controlador de recambio, sigue esta secuencia de pasos.

Debes tener preparado el nuevo módulo controlador para poder trasladar el módulo de almacenamiento en caché directamente desde el módulo controlador antiguo a la ranura correspondiente del nuevo. El resto de componentes del sistema de almacenamiento deben funcionar correctamente. Si no es así, ponte en contacto con el soporte técnico.
...
En el caso de los pares de alta disponibilidad (HA) con dos módulos controladores en el mismo chasis, el orden en el que instalas los módulos controladores es especialmente importante porque un módulo controlador intenta reiniciarse tan pronto como queda completamente encajado en el chasis.
