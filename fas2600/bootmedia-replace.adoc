...
summary: Reemplace el medio de arranque fallido en un módulo controlador FAS2600. El sistema FAS2600 solo admite procedimientos de recuperación de medios de arranque manuales. No se admite la recuperación automática de medios de arranque. 
---
= Sustituir el soporte de arranque en un sistema FAS2600
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Reemplace el medio de arranque fallido en un módulo controlador FAS2600. Este procedimiento incluye quitar el módulo del controlador del chasis, reemplazar físicamente el componente del medio de arranque, transferir la imagen de arranque al medio de reemplazo mediante una unidad flash USB y restaurar el sistema a su funcionamiento normal.
...
* Si tu sistema es autónomo, no necesitas una conexión de red, pero debes realizar un reinicio adicional al restaurar el sistema de archivos var.
...
NOTE: El puerto de destino que configures es el que usas para comunicarte con el controlador afectado desde el controlador en buen estado durante la restauración del sistema de archivos var a través de una conexión de red. También puedes usar el puerto e0M en este comando.
