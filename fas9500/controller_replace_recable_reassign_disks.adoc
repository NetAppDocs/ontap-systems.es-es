...
summary: Vuelve a cablear el sistema y reasigna los discos. 
---
= Devuelve el controlador en tu sistema FAS9500 para reanudar el funcionamiento normal
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Devuelve el control de los recursos de almacenamiento al controlador de sustitución para que tu sistema FAS9500 pueda reanudar su funcionamiento normal.
...
.. Introduce la información del sistema de destino y luego selecciona Recopilar datos.
.. Selecciona la pestaña «Cableado» y, a continuación, revisa la salida. Asegúrate de que se muestren todas las bandejas de discos y de que todos los discos aparezcan en la salida, corrigiendo cualquier problema de cableado que encuentres.
.. Comprueba el resto del cableado seleccionando la pestaña correspondiente y luego revisando la salida de Config Advisor.





NOTE: La identificación del sistema y la información sobre la asignación de discos se almacenan en el módulo NVRAM, que se encuentra en un módulo independiente del módulo controlador y no se ve afectado por la sustitución del módulo controlador.
...
Si el sistema de almacenamiento está en un par HA, el ID del sistema del nuevo módulo de controlador se asigna automáticamente a los discos cuando se produce la devolución al final del procedimiento. Debes confirmar el cambio de ID del sistema al arrancar el nodo _de sustitución_ y luego verificar que el cambio es correcto.
...
. Desde el indicador LOADER del nodo _replacement_, arranca el nodo e introduce  `y` si se te solicita que sobrescribas el ID del sistema debido a una discrepancia en el ID del sistema: `boot_ontap`
