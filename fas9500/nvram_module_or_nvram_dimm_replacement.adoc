...
---
= Sustituye el módulo NVRAM o los DIMM NVRAM en un sistema FAS9500
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
El módulo de NVRAM de tu sistema FAS9500 está compuesto por el NVRAM11 y los DIMM. Puedes reemplazar un módulo de NVRAM defectuoso o un DIMM de NVRAM para que tu sistema siga funcionando normalmente.

Para sustituir un módulo de NVRAM averiado, debes extraerlo del chasis, trasladar los módulos DIMM al módulo de recambio e instalar el módulo de NVRAM de recambio en el chasis.

Para sustituir un DIMM de NVRAM, debes extraer el módulo de NVRAM del chasis, reemplazar el DIMM defectuoso en el módulo y luego volver a instalar el módulo de NVRAM.
...
Debes confirmar el cambio de ID del sistema al arrancar el controlador de sustitución y luego verificar que el cambio es correcto.
...
+
La configuración de MetroCluster tarda unos minutos en volver al estado normal tras la sustitución, momento en el que cada controlador muestra un estado configurado, con DR Mirroring activado y un modo normal. La salida del comando  `metrocluster node show -fields node-systemid` muestra el antiguo ID del sistema hasta que la configuración de MetroCluster vuelve al estado normal.
