...
---
= Pasos de sustitución de medios posteriores al arranque para OKM, NSE y NVE: FAS9500
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Una vez marcadas las variables de entorno, debe completar los pasos específicos para restaurar la gestión de claves incorporada (OKM), el cifrado de almacenamiento de NetApp (NSE) y el cifrado de volúmenes de NetApp (NVE).
...
. Conecte el cable de consola al nodo de destino.
. Utilice la `boot_ontap` Comando en el símbolo del sistema del CARGADOR para arrancar el nodo.
...
. Mueva el cable de la consola al nodo compañero e inicie sesión como administrador.
...
. Una vez que se haya completado la devolución, compruebe el estado de la conmutación al nodo de respaldo y la devolución con el `storage failover show` y comandos show-giveback de conmutación al nodo de respaldo de almacenamiento.
+
...
. Mueva el cable de consola al nodo compañero.
. Vuelva a generar el nodo de destino mediante el `storage failover giveback -fromnode local` comando.
...
. En el símbolo del sistema clustershell, introduzca el `net int show -is-home false` comando para mostrar las interfaces lógicas que no están en su puerto y nodo de inicio.
+
...
. Mueva el cable de consola al nodo de destino y ejecute el `version -v` Comando para comprobar las versiones de ONTAP.
...
|===
. Mueva el cable de consola al nodo asociado y vuelva a proporcionar el almacenamiento del nodo de destino mediante el `storage failover giveback -fromnode local -only-cfo-aggregates true local` comando.
+
** Si el comando falla debido a un disco fallido, desactive físicamente el disco que ha fallado, pero deje el disco en la ranura hasta que se reciba un reemplazo.
