...
---
= Agrega un módulo de I/O a un sistema FAS9500
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Puedes añadir un módulo de E/S a tu sistema FAS9500, ya sea sustituyendo una NIC o un adaptador de almacenamiento por uno nuevo en un sistema con todas las ranuras ocupadas, o bien añadiendo una nueva NIC o un nuevo adaptador de almacenamiento en una ranura vacía del chasis.
...
NOTE: Asegúrate de instalar tapones en todas las ranuras de E/S que no se utilicen para evitar posibles problemas térmicos.
...
. Si estás utilizando la ranura 3 o 7 para la red, utiliza el comando  `storage port modify -node __<node name>__ -port __<port name>__ -mode network` para convertir la ranura para uso de red.
...
NIC a NIC (el mismo número de puertos)
 a| 
Las LIFs migran automáticamente cuando se apaga el módulo controlador.
