...
summary: Sustituye la batería del reloj en tiempo real, comúnmente conocida como batería de celda tipo moneda, en tu sistema de almacenamiento AFF A20, AFF A30 o AFF A50 para garantizar que los servicios y aplicaciones que dependen de una sincronización horaria precisa sigan funcionando. 
---
= Sustituye la batería del reloj en tiempo real de tu sistema AFF A20, AFF A30 o AFF A50
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Reemplace la batería del reloj de tiempo real (RTC), comúnmente conocida como batería de celda de moneda, en su sistema de almacenamiento AFF A20, AFF A30 o AFF A50 para garantizar que los servicios y las aplicaciones que dependen de una sincronización horaria precisa sigan funcionando.

.Antes de empezar
El resto de componentes del sistema de almacenamiento deben funcionar correctamente. Si no es así, ponte en contacto con https://mysupport.netapp.com/site/global/dashboard["Soporte de NetApp"] antes de continuar con este procedimiento.
...
|===
. Gira el tornillo de mariposa en sentido antihorario para aflojarlo y, a continuación, abre la tapa del controlador.
...
NOTE: Después de cambiar la batería del RTC, insertar el controlador y encender el sistema para el primer reinicio de la BIOS, verás los siguientes mensajes de error:
`RTC date/time error. Reset date/time to default`
`RTC power failure error` Estos mensajes son normales y puedes continuar con este procedimiento.

. En el controlador en buen estado, verifique la fecha y la hora:
+
`cluster date show`

+

NOTE: Si tu sistema de almacenamiento se queda bloqueado en el menú de arranque, selecciona la opción  `Reboot node` y responde _y_ cuando se te solicite, luego arranca en LOADER pulsando _Ctrl-C_.

. En el controlador dañado, en el indicador LOADER, verifique la hora y la fecha:
+
`cluster date show`

+
.. Si es necesario, modifique la fecha:
+
`set date _mm/dd/yyyy_`

.. Si es necesario, configure la hora, en GMT:
+
`set time hh:mm:ss`
...
. En el indicador LOADER, escribe  `bye` para reinicializar los módulos de E/S y otros componentes y para reiniciar el controlador.
