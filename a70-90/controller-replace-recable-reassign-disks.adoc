...
[role="lead"]
Devolver el control de los recursos de almacenamiento a la controladora de reemplazo para que su sistema AFF A70 o AFF A90 puedan reanudar el funcionamiento normal.
...
. Asegúrese de que todos los volcados de memoria en el nodo reparado se guarden en el modo avanzado:
+
`set -privilege advanced` y luego `run local partner savecore`.
