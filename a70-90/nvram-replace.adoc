...
---
= Sustituye la NVRAM en un sistema AFF A70 o AFF A90
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Reemplace la NVRAM en su sistema AFF A70 o AFF A90 cuando la memoria no volátil se vuelva defectuosa o requiera una actualización.  El proceso de reemplazo implica apagar el controlador dañado, quitar completamente la energía tirando del controlador unos centímetros hacia afuera, reemplazar el módulo NVRAM o el DIMM NVRAM , reasignar los discos y devolver la parte defectuosa a NetApp.
...
WARNING: El módulo NVRAM no es intercambiable en caliente. Antes de reemplazarlo, vuelve a colocar las unidades, apaga el nodo, desconecta la alimentación del controlador y saca el controlador unas pulgadas del chasis.
...
----
+
*** Si el comando retorna `There are no entries matching your query.` continuar<<Go-to-the-rear,ir al siguiente paso>> .
*** Si el comando devuelve otros resultados, recopile los datos de AutoSupport de ambos controladores y comuníquese con el soporte de NetApp para obtener más ayuda.
+
[source, cli]
----
...
. [[Ir a la parte trasera]]Vaya a la parte trasera del chasis.
...
--
Para sustituir los módulos DIMM de NVRAM en el módulo de NVRAM, primero debes extraer el módulo de NVRAM y luego reemplazar el DIMM objetivo.
...
+
.. Desde el controlador que funciona correctamente, devuelve el almacenamiento del controlador sustituido:
+
