...
. Desde el nodo en buen estado, restaure la devolución automática si la deshabilitó mediante el siguiente comando:
+
`storage failover modify -node local -auto-giveback true`

. Si AutoSupport está habilitado, restaure la creación automática de casos mediante el siguiente comando:
+
