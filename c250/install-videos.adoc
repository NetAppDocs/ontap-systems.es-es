---
permalink: c250/install-videos.html 
sidebar: sidebar 
keywords: aff c250, hardware installation and cabling video 
summary: En el siguiente vídeo se muestra cómo instalar y cablear el sistema nuevo. 
---
= Instala un sistema de almacenamiento AFF C250 siguiendo las instrucciones en vídeo
:allow-uri-read: 
:icons: font
:imagesdir: ../media/


[role="lead"]
Mira un vídeo de demostración sobre cómo instalar y cablear un sistema de almacenamiento AFF C250.

.Animación: Instalación y configuración de un AFF C250
video::c6906786-b302-4c14-b39b-afc50062aac5[panopto]
Si dispone de una configuración MetroCluster, utilice el contenido de instalación de MetroCluster.

https://docs.netapp.com/us-en/ontap-metrocluster/index.html["Documentación de MetroCluster"^]
