...
summary: Transfiera la imagen de arranque al medio de arranque de reemplazo en un sistema AFF A700s utilizando el medio de arranque secundario o una unidad flash USB. El sistema AFF A700s solo admite procedimientos de recuperación de medios de arranque manuales. No se admite la recuperación automática de medios de arranque. 
---
= Transfiere la imagen de arranque al soporte de arranque de tu sistema AFF A700s usando una memoria USB
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Transfiere la imagen de arranque al medio de arranque de reemplazo en un sistema AFF A700s usando el medio de arranque secundario (método principal) o una unidad flash USB (si falla la restauración del medio de arranque secundario o falta el archivo `image.tgz`).
...
== Opción 1: transferir la imagen del sistema utilizando la recuperación de copia de seguridad desde el segundo medio de arranque

Puedes instalar la imagen del sistema en el medio de arranque de reemplazo usando la imagen del segundo medio de arranque en el módulo controlador. Este es el método principal para sistemas con dos medios de arranque en el módulo controlador.

La imagen del medio de arranque secundario debe contener un archivo `image.tgz` y no debe informar de fallos. Si falta el archivo `image.tgz` o el medio de arranque informa de fallos, no puedes usar este procedimiento. Usa <<Opción 2: Transferir la imagen de arranque mediante una unidad flash USB>> en su lugar.
...
== Opción 2: Transferir la imagen de arranque mediante una unidad flash USB

Este procedimiento solo debe utilizarse si falla la restauración del medio de arranque secundario o si no se encuentra el archivo `image.tgz` en el medio de arranque secundario.

* Debe tener una unidad flash USB, formateada con FAT32, con una capacidad mínima de 4 GB.
* Una copia de la misma versión de imagen de ONTAP que la que estaba ejecutando el controlador dañado. Puedes descargar la imagen adecuada de la sección Descargas en el sitio de soporte de NetApp.
+
...
.. Guarda las variables de entorno que cambiaste con el comando `savenv`.
...
. En la controladora asociada en el nivel de privilegio avanzado, inicie la sincronización de configuración con la dirección IP registrada en el paso anterior: `system node restore-backup -node local -target-address _impaired_node_IP_address_`
. Cuando la sincronización de la configuración se haya completado sin errores, pulse `y` cuando se le solicite que confirme que el procedimiento de copia de seguridad se ha realizado correctamente.
. Verifica que las variables de entorno estén configuradas como esperas.
+
