...
summary: Debes sustituir el módulo de almacenamiento en caché del módulo controlador cuando tu sistema registre un único mensaje ASUP AutoSupport que indique que el módulo se ha desconectado. Si no lo haces, se produce una degradación del rendimiento. 
---
= Sustituye el módulo de almacenamiento en caché o el módulo de volcado de memoria en tu sistema FAS9000
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
