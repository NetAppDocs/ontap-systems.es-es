...
summary: Para completar la sustitución del controlador de tu sistema FAS50, primero restaura la configuración de NetApp Storage Encryption si es necesario e instala las licencias necesarias en el nuevo controlador. Luego, confirma que las interfaces lógicas están reportando a sus puertos de origen y realiza una comprobación del estado del clúster. Por último, registra el número de serie del nuevo controlador y luego devuelve la pieza averiada a NetApp. 
---
= Completa la sustitución del controlador de un sistema FAS50
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Para completar la controladora de reemplazo del sistema FAS50, restaure primero la configuración de cifrado de almacenamiento de NetApp (si es necesario) e instale las licencias requeridas en la controladora nueva. A continuación, confirme que las interfaces lógicas (LIF) informan a sus puertos principales y realizan una comprobación del estado del clúster. Finalmente, registre el número de serie de la nueva controladora y luego devuelva la pieza fallida a NetApp.
