...
summary: Realice el mantenimiento del hardware de su sistema de almacenamiento FAS50 para garantizar la fiabilidad a largo plazo y un rendimiento óptimo. Realice tareas de mantenimiento periódicas, como la sustitución de componentes defectuosos, para evitar tiempos de inactividad y la pérdida de datos. 
---
= Infórmate sobre el mantenimiento de tu sistema de almacenamiento FAS50
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
link:caching-module-hot-swap.html["Módulo de almacenamiento en caché"]
 a| 
Un módulo de almacenamiento en caché (módulo Flash Cache) utiliza unidades SSD de alta velocidad para almacenar datos a los que se accede con frecuencia y así recuperarlos más rápido.
