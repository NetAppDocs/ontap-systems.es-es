...
summary: Debes sustituir el NVDIMM del módulo controlador cuando tu sistema registre que la vida útil de la memoria flash está a punto de finalizar o que el NVDIMM identificado no está en buen estado. Si no lo haces, se producirá un pánico del sistema. 
---
= Sustituir un módulo NVDIMM en tu sistema ASA C800
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Reemplace el NVDIMM en su sistema ASA C800 cuando su sistema registre que la vida útil del flash está casi llegando a su fin o que el NVDIMM identificado no está en buen estado en general; no hacerlo provoca un pánico en el sistema.

.Antes de empezar
* Asegúrese de tener el NVDIMM de reemplazo que recibió de NetApp y que sea compatible con su sistema ASA C800 .
