...
---
= Vuelve a cablear el sistema y reasigna los discos en tu sistema AFF A800
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Vuelve a cablear tu sistema AFF A800 tras sustituir un controlador y reasigna los discos al controlador de recambio. Si tu sistema está en un par HA, el proceso de giveback asigna automáticamente el ID del sistema a los discos; los pasos exactos dependen de si tu sistema no utiliza cifrado o si utiliza cifrado Onboard Key Manager (OKM).
...
Si el sistema de almacenamiento está en un par HA, el ID del sistema del nuevo módulo controlador se asigna automáticamente a los discos cuando se produce la devolución al final del procedimiento. Debes confirmar el cambio de ID del sistema al arrancar el controlador _de sustitución_ y luego verificar que el sistema aplicó el cambio.

Este paso solo se aplica a los sistemas que ejecutan ONTAP en un par de alta disponibilidad.
