...
summary: Arranque la imagen de recuperación de ONTAP desde la unidad USB en un sistema AFF C190 para restaurar el medio de arranque. El sistema AFF C190 solo admite procedimientos de recuperación de medios de arranque manuales. No se admite la recuperación automática de medios de arranque. 
---
= Arranca la imagen de recuperación en un sistema AFF C190
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Arranque la imagen de recuperación de ONTAP desde la unidad USB en un sistema AFF C190 para restaurar el medio de arranque. Este procedimiento incluye arrancar desde la unidad flash USB, restaurar el sistema de archivos, verificar las variables ambientales y devolver el controlador al funcionamiento normal después del reemplazo del medio de arranque.
...
.Pasos
. Desde el símbolo DEL SISTEMA DEL CARGADOR, arranque la imagen de recuperación desde la unidad flash USB:
+
`*boot_recovery*`
