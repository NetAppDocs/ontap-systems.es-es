...
--
.. En el aviso de Loader del controlador defectuoso, configure la dirección IP del módulo del controlador dañado:
+
`ifconfig e0M - auto`

.. En el símbolo del sistema de Loader, reinicie el nodo dañado:
+
`netboot \http://_web_server_ip/path_to_web-accessible_directory_/<ontap_version>_image.tgz`

.. En el menú de inicio, seleccione la opción `7`.
.. Introduzca `n` cuando se le pida restaurar la configuración de copia de seguridad.
.. Introduzca `y` cuando se le solicite reiniciar el nodo.
.. Pulse `ctrl-c` para volver al menú de inicio.
+
Si el cifrado está activado, debe restaurarlo antes de continuar restaurando la imagen de inicio. Vaya a link:bootmedia-encryption-restore.html["Restaure el cifrado"].
