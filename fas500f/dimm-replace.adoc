...
summary: Debes sustituir un módulo DIMM en el módulo controlador cuando tu sistema registre un número creciente de códigos de corrección de errores corregibles. Si no lo haces, se produce un pánico en el sistema. 
---
= Sustituye un módulo DIMM en tu sistema FAS500f para solucionar errores de memoria
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Debes sustituir un módulo DIMM del controlador cuando tu sistema de almacenamiento detecte errores, como un número excesivo de errores CECC (códigos de corrección de errores corregibles) según las alertas del Health Monitor, o errores ECC no corregibles. Estos errores suelen deberse a un fallo en un único módulo DIMM que impide que el sistema de almacenamiento arranque ONTAP.

Si algún otro componente del sistema no funciona correctamente, ponte en contacto con soporte técnico.
...
|===
. Sujeta el módulo del controlador con ambas manos y sácalo con cuidado del chasis, luego colócalo sobre una superficie plana y estable.
...
+
Los módulos DIMM deben encajar perfectamente en el zócalo. Si un módulo DIMM no encaja bien, vuelve a insertarlo para alinearlo con el zócalo.
...
+
.. Comprueba que los brazos del mecanismo de bloqueo estén bloqueados en la posición totalmente extendida.
.. Con ambas manos, alinea y desliza suavemente el módulo del mando en los brazos del mecanismo de enclavamiento hasta que se detenga.
