...
summary: Después de instalar el nuevo dispositivo de medios de arranque en tu sistema FAS500f, puedes iniciar el proceso automatizado de recuperación de medios de arranque para restaurar la configuración desde el nodo asociado. 
---
= Inicia la imagen de recuperación en un sistema FAS500f mediante el proceso de arranque de recuperación automatizado
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Después de instalar el nuevo dispositivo de arranque en tu sistema FAS500f, puedes iniciar el proceso automatizado de recuperación del dispositivo de arranque para restaurar la configuración desde el nodo asociado. Durante el proceso de recuperación, el sistema comprueba si el cifrado está habilitado y determina el tipo de cifrado de claves en uso. Si el cifrado de claves está habilitado, el sistema te guía por los pasos adecuados para restaurarlo.
...
.El futuro
Una vez que hayas restaurado la imagen de ONTAP y el nodo esté en funcionamiento y sirviendo datos, link:bootmedia-complete-rma-bmr.html["Devuelva la pieza fallida a NetApp"].
