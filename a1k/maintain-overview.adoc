...
[role="lead"]
Mantener el hardware de su sistema de almacenamiento A1K de AFF para garantizar la fiabilidad a largo plazo y un rendimiento óptimo. Lleve a cabo tareas de mantenimiento periódicas, como la sustitución de componentes defectuosos, ya que esto ayuda a evitar el tiempo de inactividad y la pérdida de datos.

Los procedimientos de mantenimiento suponen que el sistema AFF A1K ya se ha puesto en marcha como nodo de almacenamiento en el entorno de ONTAP.
...
link:bootmedia-replace-workflow.html["Medios de arranque: recuperación manual"]
 a| 
El medio de arranque almacena un conjunto primario y secundario de archivos de imagen ONTAP que el sistema utiliza para arrancar la imagen desde una unidad USB y restaurar la configuración desde el nodo asociado.
...
link:chassis-replace-workflow.html["Chasis"]
 a| 
El chasis alberga los ventiladores, el controlador, las fuentes de alimentación y las PSU, NVRAM12, el módulo de administración del sistema, las tarjetas de E/S y los módulos de supresión.
