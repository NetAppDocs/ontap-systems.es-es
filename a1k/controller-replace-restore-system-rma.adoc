...
[role="lead"]
Para completar la sustitución de la controladora del sistema AFF A1K, primero restaure la configuración de cifrado del almacenamiento de NetApp (si es necesario) e instale las licencias requeridas en la controladora nueva. A continuación, confirme que las interfaces lógicas (LIF) informan a sus puertos principales y realizan una comprobación del estado del clúster. Finalmente, registre el número de serie de la nueva controladora y luego devuelva la pieza fallida a NetApp.
