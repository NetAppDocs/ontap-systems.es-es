...
---
= Agrega un módulo de I/O a un sistema AFF C30 o AFF C60
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Añade un módulo de E/S a tu sistema de almacenamiento AFF C30 o AFF C60 para mejorar la conectividad de red y ampliar la capacidad de tu sistema para gestionar el tráfico de datos. Puedes usar este procedimiento para añadir un módulo de E/S tanto a un controlador en funcionamiento como parte de una actualización planificada, o a un controlador con un módulo de E/S averiado durante el mantenimiento.
...
Puedes realizar este procedimiento en cualquiera de los controladores del par de alta disponibilidad (HA), sin importar si el módulo de E/S del controlador está operativo o averiado en ese momento. Como no se puede añadir un módulo de E/S a un controlador en funcionamiento, tienes que apagar el controlador de destino antes de instalar el módulo. El partner de alta disponibilidad se encarga de las operaciones de almacenamiento durante el procedimiento para que los datos sigan estando disponibles para los clientes.

* Si el controlador de destino está *en funcionamiento* (incorporación prevista), sigue los pasos de apagado tal y como se indican, considerando en dicho procedimiento que el controlador de destino tiene un módulo de E/S averiado.
* Si el controlador de destino tiene un módulo de E/S defectuoso, sigue los pasos de apagado tal y como se indican.
...
Toma el control y apaga el controlador de destino para que su socio de alta disponibilidad (HA) siga proporcionando datos mientras añades el módulo de E/S. Los pasos enlazados se refieren al controlador que se va a apagar como el que aloja el módulo de E/S defectuoso. Estos pasos se aplican tanto si vas a tomar el control de un controlador para una incorporación planificada como si vas a apagar un controlador que ya tiene un módulo de E/S defectuoso.
...
.Antes de empezar
* Consulta la  https://hwu.netapp.com["Hardware Universe de NetApp"^] para asegurarte de que el nuevo módulo de E/S es compatible con tu sistema de almacenamiento y con la versión de ONTAP que usas.
