...
[role="lead"]
Restaure la configuración de cifrado de almacenamiento de NetApp (si es necesario), instale licencias para la controladora de reemplazo, compruebe que las interfaces lógicas (LIF) están informando a su puerto de inicio, realice una comprobación del estado del clúster, registre el número de serie de la controladora de reemplazo y vuelva a colocar la pieza fallida a NetApp para completar el paso final en el procedimiento de sustitución de las controladoras AFF C30 y C60.
