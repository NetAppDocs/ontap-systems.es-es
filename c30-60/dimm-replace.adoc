...
summary: 'Reemplace un DIMM en el módulo del controlador cuando el sistema registre un número creciente de códigos de corrección de errores corregibles (ECC); si no lo hace, se producirá una alarma en el sistema.' 
---
= Sustituya un módulo DIMM: AFF C30 y AFF C60
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Reemplace un DIMM en su sistema AFF C30 o AFF C60 si se detectan demasiados errores de memoria corregibles o que no se pueden corregir. Estos errores pueden impedir que el sistema de almacenamiento arranque la ONTAP. El proceso de sustitución implica apagar la controladora deteriorada, extraerla, sustituir el DIMM, volver a instalar la controladora y, a continuación, devolver la pieza averiada a NetApp.
