...
summary: Para restablecer el funcionamiento completo de tu sistema AFF A900, vuelve a cablear el almacenamiento, restaura la configuración de NetApp Storage Encryption si es necesario e instala licencias para el nuevo controlador. 
---
= Completa la sustitución del controlador de un sistema AFF A900
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Para completar la sustitución del controlador de tu sistema AFF A900, restaura la configuración de cifrado de almacenamiento de NetApp (si es necesario), confirma que las interfaces lógicas (LIF) están reportando a sus puertos de origen, realiza una comprobación del estado del clúster y devuelve la pieza defectuosa a NetApp.
