---
permalink: afx-2k/power-supply-replace.html 
sidebar: sidebar 
keywords: afx 2k, replace a power supply 
summary: Sustituye la fuente de alimentación de CA de tu sistema de almacenamiento afx 2k cuando falle o presente algún fallo, asegurándote de que tu sistema siga recibiendo la energía necesaria para un funcionamiento estable. El proceso de sustitución consiste en desconectar la fuente de alimentación en cuestión, desenchufar el cable de alimentación, retirar la fuente de alimentación defectuosa, instalar la de repuesto y luego volver a conectarla a la fuente de energía. 
---
= Reemplaza una fuente de alimentación - AFX 2K
:allow-uri-read: 
:icons: font
:imagesdir: ../media/


[role="lead"]
Sustituye la fuente de alimentación de CA (PSU) de tu sistema de almacenamiento AFX 2K cuando falle o presente algún fallo, asegurándote de que tu sistema siga recibiendo la alimentación necesaria para un funcionamiento estable. El proceso de sustitución consiste en desconectar la PSU en cuestión, desenchufar el cable de alimentación, retirar la PSU defectuosa e instalar la PSU de reemplazo, y luego volver a conectarla a la fuente de alimentación.

.Acerca de esta tarea
* Este procedimiento se escribe para sustituir una fuente de alimentación por vez.
+

IMPORTANT: No mezcle las PSU con diferentes niveles de eficiencia. Sustituya siempre como por ejemplo.

* Las fuentes de alimentación son redundantes e intercambiables en caliente; no es necesario tomar el control del controlador para realizar esta tarea.



CAUTION: Lleva siempre una muñequera con toma de tierra conectada a un punto de tierra verificado durante los procedimientos de instalación y mantenimiento. No seguir las precauciones ESD adecuadas puede causar daños permanentes a los nodos controladores, las estanterías de almacenamiento y los switches de red.

.Pasos
. Identifique la fuente de alimentación que desea sustituir, basándose en los mensajes de error de la consola o a través del LED rojo de fallo de la fuente de alimentación.
. Desconecte la fuente de alimentación:
+
.. Abra el retenedor del cable de alimentación y, a continuación, desconecte el cable de alimentación de la fuente de alimentación.


. Retire la PSU girando el asa hacia arriba, presione la lengüeta de bloqueo y tire de la PSU para extraerla del módulo de la controladora.
+

CAUTION: La fuente de alimentación es compacta.  Utilice ambas manos para sostenerlo durante la extracción para evitar que se suelte del módulo del controlador y provoque lesiones.

+
image::../media/drw_afx_2k_psu_remove_replace_ieops-2882.svg[Sustituya la PSU]

+
[cols="1,4"]
|===


 a| 
image:../media/icon_round_1.png["Número de llamada 1"]
 a| 
Lengüeta de bloqueo de PSU de terracota

|===
. Instale la fuente de alimentación de repuesto en el módulo del controlador:
+
.. Con ambas manos, sujete y alinee los bordes de la PSU de repuesto con la abertura del módulo del controlador.
.. Empuje con cuidado la PSU hacia el módulo de la controladora hasta que la lengüeta de bloqueo haga clic en su lugar.
+
Las fuentes de alimentación sólo se acoplarán correctamente al conector interno y se bloquearán de una manera.

+

NOTE: Para evitar dañar el conector interno, no ejerza demasiada fuerza al deslizar la fuente de alimentación hacia el sistema.



. Vuelva a conectar el cableado de la PSU:
+
.. Vuelva a conectar el cable de alimentación a la fuente de alimentación.
.. Fije el cable de alimentación a la fuente de alimentación con el retenedor del cable de alimentación.


+
Una vez restaurada la alimentación de la fuente de alimentación, el LED de estado debería ser verde.

. Devuelva la pieza que ha fallado a NetApp, como se describe en las instrucciones de RMA que se suministran con el kit. Consulte https://mysupport.netapp.com/site/info/rma["Devolución de piezas y sustituciones"^] la página para obtener más información.

