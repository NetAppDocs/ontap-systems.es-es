...
summary: Debes sustituir el NVDIMM en el módulo controlador cuando tu sistema registre que la vida útil de la memoria flash está a punto de finalizar o que el NVDIMM identificado no está sano en general. 
---
= Sustituye el NVDIMM en un sistema FAS8300 o FAS8700
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
El resto de componentes del sistema deben funcionar correctamente. Si no es así, debes ponerte en contacto con soporte técnico.
...
Para sustituir el NVDIMM, debes localizarlo en el módulo controlador utilizando el mapa de FRU que se encuentra en la parte superior del riser de la ranura 1.
...
* Aunque el contenido del NVDIMM está cifrado, se recomienda borrar dicho contenido antes de sustituirlo. Para obtener más información, consulta el  https://mysupport.netapp.com/info/web/ECMP1132988.html["Declaración de volatilidad"] en el sitio de soporte de NetApp.
+
...
+
El NVDIMM encaja perfectamente en la ranura, pero debería introducirse con facilidad. Si no es así, vuelve a alinear el NVDIMM con la ranura y vuelve a insertarlo.
