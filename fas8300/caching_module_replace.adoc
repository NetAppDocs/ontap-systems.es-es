...
+
.. Borre los datos del módulo de almacenamiento en caché: `system controller flash-cache secure-erase run -node _node_name_ -device-id _device_id_`
.. Compruebe que los datos se han borrado del módulo de almacenamiento en caché: `system controller flash-cache secure-erase show -node _node_name_`
+
...
. Seleccione *sistema de prueba de esfuerzo* en el menú que se muestra.
. Seleccione **NVME M.2 Drive Stress** en el menú que se muestra.
