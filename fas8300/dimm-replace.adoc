...
summary: Debes sustituir un módulo DIMM en el controlador cuando tu sistema de almacenamiento encuentre errores como códigos de corrección de errores corregibles excesivos o errores no corregibles, normalmente causados por un fallo de un único módulo DIMM que impide que el sistema de almacenamiento arranque ONTAP. 
---
= Sustituye un módulo DIMM en tu sistema FAS8300 o FAS8700 para solucionar errores de memoria
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Debes sustituir un módulo DIMM del controlador cuando tu sistema de almacenamiento detecte errores, como un número excesivo de errores CECC (Código de corrección de errores corregibles) según las alertas del Health Monitor, o errores ECC no corregibles. Estos errores suelen deberse a un fallo en un único módulo DIMM que impide que el sistema de almacenamiento arranque ONTAP.

El resto de componentes del sistema deben funcionar correctamente. Si algún componente no funciona correctamente, tienes que ponerte en contacto con el soporte técnico.
...
El número y la ubicación de los módulos DIMM en tu sistema dependen del modelo de tu sistema. Consulta el mapa de FRU en el conducto de aire para más información.
...
+
El DIMM encaja perfectamente en la ranura, pero debería introducirse con facilidad. Si no es así, vuelve a alinear el DIMM con la ranura y vuelve a insertarlo.
...
NOTE: Conectas el resto de los cables al módulo controlador más adelante en este procedimiento.

. Complete la instalación del módulo del controlador:
+
.. Empuja con firmeza el módulo controlador hacia el chasis utilizando los pestillos de bloqueo hasta que los pestillos empiecen a levantarse.
+
