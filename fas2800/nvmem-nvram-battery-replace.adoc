...
summary: Para sustituir una batería NVMEM en tu sistema FAS2820, tienes que extraer el módulo controlador, abrirlo, cambiar la batería y luego cerrarlo y volver a instalar el módulo controlador. 
---
= Sustituye la batería NVMEM de tu sistema FAS2820
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Para sustituir una batería NVMEM en tu sistema FAS2820, tienes que extraer el módulo controlador, abrirlo, cambiar la batería y luego cerrarlo y volver a instalar el módulo controlador.

Si algún otro componente del sistema no funciona correctamente, ponte en contacto con https://mysupport.netapp.com/site/global/dashboard["Soporte de NetApp"].
...
NOTE: El LED verde NV de la placa frontal comienza a parpadear cuando se corta la alimentación del controlador si el sistema se encontraba en el estado «a la espera de devolución» o si el sistema no se ha transferido o detenido correctamente (datos no confirmados). Si el módulo controlador afectado no ha sido transferido correctamente al módulo controlador asociado, ponte en contacto con https://mysupport.netapp.com/site/global/dashboard["Soporte de NetApp"]

+
** Si el LED verde de estado de NV comienza a parpadear cuando se extrae el módulo de controlador del chasis:
+
*** Confirma que el controlador haya sido sustituido correctamente por el módulo controlador asociado, o que el controlador averiado muestre _waiting for giveback_. Si es así, puedes ignorar el LED parpadeante y continuar retirando el controlador averiado del chasis.


** Si el LED verde NV está apagado, ya puedes terminar de retirar el controlador defectuoso del chasis.
...
.Pasos
. Si usted no está ya conectado a tierra, correctamente tierra usted mismo.
. Retira el módulo controlador del chasis.
...
. Instala la batería de recambio:
+
