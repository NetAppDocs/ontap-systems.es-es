...
summary: El soporte de arranque de tu sistema FAS2820 almacena datos esenciales de firmware y configuración. Para reemplazarlo, tienes que quitar y abrir el módulo de controlador con problemas, localizar y reemplazar el soporte de arranque, transferir la imagen de arranque a una unidad USB y luego arrancar el controlador. 
---
= Sustituye el soporte de arranque en un sistema FAS2820 durante el proceso de recuperación de arranque manual
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
El soporte de arranque de tu sistema FAS2820 almacena datos esenciales de firmware y configuración. Para reemplazarlo, tienes que quitar y abrir el módulo de controlador con fallo, localizar y reemplazar el soporte de arranque, transferir la imagen de arranque a una unidad USB, insertar la unidad USB en el controlador y luego arrancar el controlador.
...
Localiza el soporte de arranque, que está debajo de la tarjeta mezzanine en el módulo del controlador, y sigue las instrucciones para reemplazarlo.
...
+
.. Utiliza https://kb.netapp.com/on-prem/ontap/DM/Encryption/Encryption-KBs/How_to_determine_if_the_running_ONTAP_version_supports_Data_At_Rest_Encryption["Cómo determinar si tu versión de ONTAP admite NetApp Volume Encryption (NVE)"^] para comprobar si actualmente se admite el cifrado de volumen.
+
