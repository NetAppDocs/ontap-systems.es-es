...
summary: Debes sustituir el NVDIMM en el módulo controlador cuando tu sistema registre que la vida útil de la memoria flash está a punto de finalizar o que el NVDIMM identificado no está sano en general. Si no lo haces, se produce un pánico en el sistema. 
---
= Sustituir un módulo NVDIMM en tu sistema ASA C400
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Sustituye un NVDIMM en tu sistema ASA C400 cuando el sistema de almacenamiento detecte un número excesivo de errores ECC corregibles o errores ECC no corregibles, que normalmente se deben a una única falla de NVDIMM.
